...
//Returns: Nothing
void* pthreads_convolute(void* rank){
    int row,pix,bit,span;
    long my_rank = (long) rank;
    long my_n = srcImage.height / N;
    row = my_rank * my_n;
    int end_row;
    end_row = row + my_n;
    if (my_rank == N - 1)
	    end_row = end_row + (srcImage.height % N);
...
int main(int argc,char** argv){
    ...
    for (thread = 0; thread < N; thread++){
	    pthread_create(&thread_handles[thread], NULL, &pthreads_convolute, (void*) thread);
    }
    for (thread = 0; thread < N; thread++){
